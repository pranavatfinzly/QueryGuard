"""Stages 1 and 8 — PR ingest and comment delivery.

Wraps PyGithub. Two things this module owns:

- Fetching the PR diff and base/head SHAs.
- Upserting a single **idempotent, tagged** Markdown comment. The comment
  carries :data:`COMMENT_MARKER` as an HTML comment so re-runs edit the existing
...
from modules.models import Report, RunContext
...
def fetch_pull_request(repo: str, pr_number: int) -> RunContext:
    ...
    raise NotImplementedError(
        "github_client.fetch_pull_request is not implemented yet"
    )


def fetch_diff(context: RunContext) -> str:
    """Fetch the PR's unified diff."""
    raise NotImplementedError("github_client.fetch_diff is not implemented yet")
...
def upsert_report_comment(context: RunContext, report: Report) -> int:
    ...
    """
    raise NotImplementedError(
        "github_client.upsert_report_comment is not implemented yet"
    )
